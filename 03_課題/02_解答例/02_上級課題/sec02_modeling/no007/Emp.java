...
public class Emp {
	...
	/**
	 * 【追加】従業員表(Emp)クラスのコンストラクタで、部門表Deptを指定する
	 */
	...
	public void insert(Row insRow) {
		...
		// 【追加】従業員表(Emp)にレコード(Row)を追加するとき、
		//  Dept表に存在しない部門番号をもつレコードはEmp表に挿入できないように仕様を変更
		RowDept[] rowDept = dept.selectAll();
		boolean isDeptno = false;
		for(int i=0; i< rowDept.length; i++) {
			if(insRow.getDeptno() == rowDept[i].getDeptno()) {
				isDeptno = true;
				break;
			}
		}
		...
		if(!isDeptno) {
			System.out.println("部門番号が部門表に存在しません");
			return;
		}
		//【追加】ここまで //
