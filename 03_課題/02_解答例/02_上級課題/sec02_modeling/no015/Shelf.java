...
public class Shelf {
	 ...
	 * @throws SameNameShelfEntryAlreadyException
	 */
	public void add(ShelfEntry entry) throws FullShelfException, SameNameShelfEntryAlreadyException {

		// ①Shellクラス内部のMapにShelfEntryオブジェクトを10個以上格納しようとした時に
		// 独自例外Aをスロー
		if(shelfEntryMap.size() >= 10) {
			throw new FullShelfException();
		}

		// ②同じ名前のShelfEntryオブジェクトを格納しようとした時に
		// 独自例外Bをスロー
		if(shelfEntryMap.get(entry.getName()) != null) {
			throw new SameNameShelfEntryAlreadyException();
		}
